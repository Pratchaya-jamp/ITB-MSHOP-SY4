describe(`TC-FE-PBI6-ADD-BRAND-4\n 
    Test Scenario : normal - all fields with max field size`, () => {
    ...
    it('should enable the save button after adding data (td-3).',()=>{
        cy.get('.itbms-manage-brand').should('exist').as('manageBrand'); 
        cy.get('@manageBrand').click();
        cy.wait(100)
   ...
   it(`should red irect to previous when clicking the save button and show a message "The band has been added.\n "
       shoud contain the new brand "Techno" in the brand list page`,()=>{
        ...
        cy.get('.itbms-message').contains('The brand has been successfully added!')
        cy.get('.itbms-name').contains('Meizu00001Meizu00002Meizu00003')
    })
