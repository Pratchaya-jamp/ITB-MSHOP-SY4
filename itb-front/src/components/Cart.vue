...
<template>
    <div :class="themeClass" class="min-h-screen font-sans transition-colors duration-300">
        <div class="container mx-auto px-6 py-12">
            <h1 class="text-4xl font-extrabold tracking-tight mb-10 text-slate-900 dark:text-white">Shopping Cart</h1>

            <div v-if="cartItems.length > 0" class="grid lg:grid-cols-3 lg:gap-12">
                <div class="lg:col-span-2 space-y-6">
                    <div class="flex items-center justify-between p-4 rounded-xl" :class="theme === 'dark' ? 'bg-gray-800/50' : 'bg-slate-100'">
                        <div class="flex items-center space-x-4">
                            ...
                            <label @click="toggleSelectAll" class="font-semibold cursor-pointer">Select All ({{ cartItems.length }} items)</label>
                        </div>
                    ...
                    <div v-for="group in groupedCartItems" :key="group.sellerId" class="p-6 rounded-2xl space-y-4" :class="theme === 'dark' ? 'bg-gray-800/50' : 'bg-white shadow-sm'">
                        <div class="flex items-center space-x-4 pb-4 border-b" :class="theme === 'dark' ? 'border-white/10' : 'border-slate-200'">
                            <input type="checkbox" :checked="group.sellerAllSelected" @change="toggleSelectSeller(group.sellerId)" class="form-checkbox h-5 w-5 rounded itbms-select-seller-all" />
                            <div class="flex items-center font-bold itbms-select-nickname">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2 text-indigo-500 dark:text-indigo-400" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd" /></svg>
                                <span class="itbms-nickname">{{ group.sellernickname }}</span>
                            </div>
                        </div>

                        <div v-for="item in group.items" :key="item.saleItemId" class="flex items-center space-x-4 pt-4 itbms-item-row">
                            <input type="checkbox" v-model="item.selected" class="form-checkbox h-5 w-5 rounded flex-shrink-0 itbms-select-item" />
                            <img src="/phone/iPhone.png" :alt="item.description" class="w-20 h-20 object-contain rounded-lg flex-shrink-0 itbms-item-image" :class="theme === 'dark' ? 'bg-gray-700/50' : 'bg-slate-100'" />
                            
                            <div class="flex-grow">
                                <p class="font-bold itbms-item-name">{{ item.model }}</p>
                                <p class="text-sm itbms-item-description" :class="theme === 'dark' ? 'text-slate-400' : 'text-slate-500'">{{ item.description }}</p>
                                <p class="font-semibold mt-1 text-indigo-600 dark:text-indigo-400">{{ formatPrice(item.price) }} ฿</p>
                            </div>

                            <div class="flex items-center space-x-2 flex-shrink-0 itbms-item-quantity">
                                <button @click="updateQuantity(item.saleItemId, -1)" :disabled="item.quantity <= 1" class="p-2 rounded-full itbms-dec-qty-button disabled:opacity-50 disabled:cursor-not-allowed" :class="theme === 'dark' ? 'bg-gray-700 hover:bg-gray-600' : 'bg-slate-200 hover:bg-slate-300'"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M20 12H4" /></svg></button>
                                <span class="w-8 text-center font-semibold itbms-item-quantity-input">{{ item.quantity }}</span>
                                <button @click="updateQuantity(item.saleItemId, 1)" :disabled="item.maxquantity && item.quantity >= item.maxquantity" class="p-2 rounded-full itbms-inc-qty-button disabled:opacity-50 disabled:cursor-not-allowed" :class="theme === 'dark' ? 'bg-gray-700 hover:bg-gray-600' : 'bg-slate-200 hover:bg-slate-300'"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" /></svg></button>
                            </div>
                            
                            <button @click="deleteItemFromCart(item.saleItemId)" class="p-2 rounded-full flex-shrink-0 itbms-delete-item-button" :class="theme === 'dark' ? 'text-slate-400 hover:bg-gray-700 hover:text-red-500' : 'text-slate-500 hover:bg-slate-200 hover:text-red-500'"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
                        </div>
                    </div>
                </div>
                ...
                <div class="lg:col-span-1">
                    <div class="sticky top-28 space-y-6">
                        <div class="p-6 rounded-2xl" :class="theme === 'dark' ? 'bg-gray-800/50' : 'bg-white shadow-sm'">
                            <h2 class="text-2xl font-bold mb-6 text-slate-900 dark:text-white">Order Summary</h2>
                            <div class="space-y-4">
                                <div>
                                    <label class="block mb-2 text-sm font-semibold">Ship To</label>
                                    <textarea rows="3" v-model="shippingAddress" :disabled="!canFillAddress" class="w-full p-3 rounded-lg text-sm resize-none focus:ring-2 focus:ring-indigo-500 border-0 outline-none transition itbms-shipping-address disabled:opacity-50 disabled:cursor-not-allowed" :class="theme === 'dark' ? 'bg-gray-700/50' : 'bg-slate-100'" placeholder="Address, Street, City, Postal Code..."></textarea>
                                </div>
                                <div>
                                    <label class="block mb-2 text-sm font-semibold">Order Note</label>
                                    <textarea rows="2" v-model="orderNote" :disabled="!canFillAddress" class="w-full p-3 rounded-lg text-sm resize-none focus:ring-2 focus:ring-indigo-500 border-0 outline-none transition itbms-order-note disabled:opacity-50 disabled:cursor-not-allowed" :class="theme === 'dark' ? 'bg-gray-700/50' : 'bg-slate-100'" placeholder="Optional: e.g. special delivery instructions"></textarea>
                                </div>
                            </div>
                        </div>
                        <div class="p-6 rounded-2xl" :class="theme === 'dark' ? 'bg-gray-800/50' : 'bg-white shadow-sm'">
            ...
            <div v-else class="text-center py-20">
                <svg xmlns="http://www.w3.org/2000/svg" class="mx-auto h-24 w-24" :class="theme === 'dark' ? 'text-gray-700' : 'text-slate-300'" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1"><path stroke-linecap="round" stroke-linejoin="round" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
                <h2 class="mt-6 text-2xl font-bold text-slate-900 dark:text-white">Your cart is empty</h2>
                <p class="mt-2 text-slate-500 dark:text-slate-400">Looks like you haven't added anything to your cart yet.</p>
                <button @click="() => router.push('/sale-items')" class="mt-8 px-8 py-3 bg-indigo-600 text-white font-semibold rounded-full shadow-lg shadow-indigo-500/20 hover:bg-indigo-700 transition-all duration-300 transform hover:-translate-y-1">
                    Continue Shopping
                </button>
            </div>
        </div>
        ...
        <transition name="bounce-popup">
             <div v-if="showConfirmationOrder" class="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
                <div class="rounded-2xl p-8 shadow-xl text-center" :class="theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-white text-black'">
                    ...
                    <div class="flex justify-center gap-4">
                        <button @click="confirmPlaceOrder" class="itbms-confirm-button bg-green-500 text-white font-semibold rounded-lg px-6 py-2 transition-all duration-300 hover:bg-green-600 active:scale-95">Confirm</button>
                        <button @click="cancelPlaceOrder" class="itbms-cancel-button bg-gray-500 text-white font-semibold rounded-lg px-6 py-2 transition-all duration-300 hover:bg-gray-600 active:scale-95">Cancel</button>
                    </div>
                </div>
             </div>
        </transition>
        ...
        <transition name="fade-background">
            <div v-if="isLoading" class="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
                <div class="p-8 rounded-2xl shadow-xl text-center" :class="theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-white text-black'">
                    <svg class="animate-spin h-8 w-8 text-indigo-500 mx-auto mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" /><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4l3.5-3.5L12 0v4a8 8 0 100 16v-4l-3.5 3.5L12 24v-4a8 8 0 01-8-8z" /></svg>
