  ...
  <div class="Itbms-sale-items-page bg-white min-h-screen">
    <div class="Itbms-header container mx-auto py-8 flex items-center justify-between">
      ...
      <div class="flex-grow flex justify-center">
        <div
          class="Itbms-search-bar flex items-center rounded-md border border-gray-300 focus-within:border-blue-500 w-full max-w-sm">
          <input type="text" placeholder="Search..." v-model="searchQuery"
            class="Itbms-search-input py-2 px-3 w-full focus:outline-none rounded-l-md text-black" />
      ...
      <div class="Itbms-icons flex flex-col items-end space-y-2">
        <div class="flex items-center space-x-4">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-black cursor-pointer" fill="none"
            viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
          </svg>
          <svg version="1.1" xmlns="http://www.w3.org/2000/svg" x="0" y="0" class="h-8 w-8 text-black cursor-pointer"
            viewBox="0 0 128 128">
            <g>
              <path
    ...
    <div class="ml-[8%] flex flex-col md:flex-row items-start justify-between mb-4 gap-4">
      ...
      <div class="itbms-filter-container flex-grow flex justify-center w-full md:w-auto">
        <div
          class="itbms-brand-filter flex items-center rounded-md border border-gray-300 focus-within:border-blue-500 w-full max-w-sm">
          <div class="flex flex-grow flex-wrap items-center gap-1 p-2 min-h-[40px] max-h-[40px] overflow-y-auto">
            <span v-for="brand in selectedBrands" :key="brand"
              class="itbms-filter-item bg-blue-100 text-blue-800 text-sm font-medium px-2.5 py-0.5 rounded-full flex items-center whitespace-nowrap">
              {{ brand }}
              <button @click.stop="removeBrandFromFilter(brand)"
                class="itbms-filter-item-clear ml-1 text-blue-800 hover:text-blue-600 focus:outline-none">
          ...
          <button @click="toggleBrandFilterModal"
            class="itbms-brand-filter-button bg-gray-100 hover:bg-gray-200 p-2 rounded-r-none focus:outline-none">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-600" fill="none" viewBox="0 0 24 24"
              stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <!-- Price Filter -->
        <div class="itbms-price-filter flex items-center rounded-md border border-gray-300 w-full max-w-sm">
          <div class="flex flex-grow flex-wrap items-center gap-1 p-2 min-h-[40px] max-h-[40px] overflow-y-auto">
            <!-- Example tag -->
            <span
              class="itbms-filter-item bg-green-100 text-green-800 text-sm font-medium px-2.5 py-0.5 rounded-full flex items-center whitespace-nowrap">
              $100 - $500
              <button class="itbms-filter-item-clear ml-1 text-green-800 hover:text-green-600 focus:outline-none">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" viewBox="0 0 20 20" fill="currentColor">
                  <path fill-rule="evenodd"
                    d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                    clip-rule="evenodd" />
            ...
            <input type="text" placeholder="Filter by price" readonly
              class="itbms-price-filter-input h-full w-full px-3 py-0 flex-grow focus:outline-none text-black bg-white cursor-pointer" />
          </div>
          <button class="itbms-price-filter-button bg-gray-100 hover:bg-gray-200 p-2 rounded-r-none focus:outline-none">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-600" fill="none" viewBox="0 0 24 24"
              stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V19l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" />
        ...
        <!-- Storage Size Filter -->
        <div class="itbms-storage-size-filter flex items-center rounded-md border border-gray-300 w-full max-w-sm">
          <div class="flex flex-grow flex-wrap items-center gap-1 p-2 min-h-[40px] max-h-[40px] overflow-y-auto">
            <!-- Example tag -->
            <span
              class="itbms-filter-item bg-purple-100 text-purple-800 text-sm font-medium px-2.5 py-0.5 rounded-full flex items-center whitespace-nowrap">
              256GB
              <button class="itbms-filter-item-clear ml-1 text-purple-800 hover:text-purple-600 focus:outline-none">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" viewBox="0 0 20 20" fill="currentColor">
                  <path fill-rule="evenodd"
                    d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                    clip-rule="evenodd" />
            ...
            <input type="text" placeholder="Filter by storage size" readonly
              class="itbms-storage-size-filter-input h-full w-full px-3 py-0 flex-grow focus:outline-none text-black bg-white cursor-pointer" />
          </div>
          <button
            class="itbms-storage-size-filter-button bg-gray-100 hover:bg-gray-200 p-2 rounded-r-none focus:outline-none">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-600" fill="none" viewBox="0 0 24 24"
              stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V19l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" />
        ...
        <button @click="clearAllBrandFilters()"
          class="itbms-brand-filter-clear bg-red-500 text-white hover:bg-transparent hover:text-red-500 border-2 border-red-500 p-2 rounded-r-md focus:outline-none transition-colors duration-300">
          Clear
        </button>
      </div>
      ...
      <div class="flex items-center space-x-4 mr-[5%]">
        ...
        <button v-if="isGridView" @click="sortBrandDescending"
          ...
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
            stroke="currentColor" class="w-6 h-6">
            <path stroke-linecap="round" stroke-linejoin="round"
              d="M3 4.5h14.25M3 9h9.75M3 13.5h9.75m4.5 4.5V8.25m0 0 3 3m-3-3-3 3" />
          </svg>
    ...
    <div class="ml-[6%] mb-6 flex items-center gap-3">
      ...
      <select id="page-size" v-model="pageSize" class="itbms-page-size border-2 border-blue-500 bg-white rounded-md px-3 py-2 text-sm font-semibold text-blue-700
    ...
    <transition name="modal-fade">
      <div v-if="showBrandFilterModal"
        ...
        <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md mx-4">
          ...
          <div
            class="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-2 max-h-60 overflow-y-auto border border-gray-300 rounded-md p-2">
            ...
            <p v-if="availableBrands.length === 0" class="text-gray-500 text-center py-4 w-full col-span-full">No brands
              available.</p>
          </div>
          <div class="mt-4 flex justify-end space-x-2">
            <button @click="showBrandFilterModal = false"
              class="bg-gray-300 text-gray-800 px-4 py-2 rounded-md hover:bg-gray-400 transition-colors duration-300">Close</button>
            <button @click="clearAllBrandFilters()"
              class="bg-red-500 text-white px-4 py-2 rounded-md hover:bg-red-600 transition-colors duration-300">Clear
              All</button>
          </div>
        </div>
      </div>
    ...
    <div v-if="isGridView" class="p-6">
      ...
      <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
        <div v-for="(item, index) in items" :key="item.id"
          ...
          <img :src="'/sy4/phone/iPhone.jpg'" alt="phone" class="w-full h-40 object-contain mb-4" />
          <div class="itbms-brand font-semibold">{{ item.brandName }}</div>
          <div class="itbms-model text-sm">{{ item.model }}</div>
          <span class="itbms-ramGb text-sm">{{ item.ramGb || '-' }}</span>/<span class="itbms-storageGb text-sm">{{
            item.storageGb || '-' }}</span> <span class="itbms-storageGb-unit text-sm">GB</span>
          <div class="itbms-price mt-2 font-bold text-lg">{{ item.price.toLocaleString() }}</div>
          <div class="itbms-price-unit text-sm">Baht</div>
        </div>
      </div>
    </div>
    ...
    <div v-else class="p-6">
      ...
      <table v-else class="w-full border-collapse border text-sm text-left text-black">
        ...
        <tbody>
          <tr v-for="item in items" :key="item.id" class="itbms-row hover:bg-gray-50 transition">
      ...
      <div class="bg-white text-black rounded-lg p-6 shadow-lg text-center">
        ...
        <div class="flex justify-center gap-4">
          <button @click="confirmDelete"
            class="itbms-confirm-button bg-green-500 text-white border-2 border-green-500 rounded-md px-4 py-2 cursor-pointer transition-colors duration-300 hover:bg-transparent hover:text-green-500">Yes</button>
          <button @click="cancelDeleteItem"
            class="itbms-cancel-button bg-red-500 text-white border-2 border-red-500 rounded-md px-4 py-2 cursor-pointer transition-colors duration-300 hover:bg-transparent hover:text-red-500">No</button>
        </div>
      </div>
    </div>
      ...
      <div v-if="showAddSuccessPopup"
        ...
        <div class="bg-white text-black rounded-lg p-6 shadow-lg text-center">
          ...
          <button @click="closeSuccessPopup"
            class="bg-blue-500 text-white border-2 border-blue-500 rounded-md px-4 py-2 cursor-pointer transition-colors duration-300 hover:bg-transparent hover:text-blue-500">Done</button>
        </div>
      </div>
    </transition>
    <transition name="bounce-popup">
    ...
    <div :class="totalPages === 1 ? 'invisible' : 'visible'" class="flex justify-center mt-6 flex-wrap gap-2">
      <!-- First -->
      <button @click="() => { lastAction = 'first'; goToPage(0) }" :disabled="currentPage === 0" :class="[
      ...
      <!-- Prev -->
      <button @click="() => { lastAction = 'prev'; goToPage(currentPage - 1) }" :disabled="currentPage === 0" :class="[
      ...
      <button @click="() => { lastAction = 'next'; goToPage(currentPage + 1) }"
        :disabled="currentPage === totalPages - 1" :class="[
      ...
      <!-- Last -->
      <button @click="() => { lastAction = 'last'; goToPage(totalPages - 1) }"
        :disabled="currentPage === totalPages - 1" :class="[
...
.itbms-bg {
  ...
  /* opacity 0.3 = โปร่งนุ่มขึ้น */
  backdrop-filter: blur(2px);
  /* เพิ่ม blur ด้านหลังให้หรู */
}
